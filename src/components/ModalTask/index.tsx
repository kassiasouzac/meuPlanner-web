...
import { FaBookReader, FaBoxes, FaCalendar, FaCarCrash, FaClock, FaGamepad, FaList, FaObjectGroup, FaPeopleCarry, FaPrayingHands, FaSpinner, FaTextWidth, FaTools } from 'react-icons/fa';
...
export function ModalOrder({ isOpen, categories, onRequestClose, task}: ModalOrderProps) {
...
setLoading(false)
  ...
  const apiClient = setupAPIClient();
   const response = await apiClient.delete('/task/remove', {
    params: {
      taskId: id,
    }
   })
  ...
  async function handleOpenModalView(id: string, categoryId: string) {
  ...
  function RenderList(item: TaskItemProps) {
    ...
    return (
      ...
      <div className={styles.itemArea}>
        ...
        <div className={styles.itemBorder}>
          ...
          <div className={styles.itemLineArea}>
            ...
            <FaCalendar size={30} fill="#FF985F" />
            <div className={styles.item}>
             {data[0]}
            </div>
          </div>
     ...
     <button className={styles.buttonOrder} onClick={ () => handleOpenModalView(item.id, item.categoryId)}>
          Editar Tarefa
        </button>
